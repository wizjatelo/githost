...
const CustomerDashboard = () => {
  ...
  const [userProfile, setUserProfile] = useState({
    ...
    loyaltyPoints: 450,
    loyaltyTier: 'Silver',
    nextTier: 'Gold (500 points needed)',
    totalSpent: 1250.75,
...
useEffect(() => {
  const fetchPromoBanners = async () => {
    try {
      ...
      const response = await fetch('https://api.yourdomain.com/promotions');
      ...
      // Optionally keep default banners if API fails
    }
  };
    ...
    try {
      ...
      setProducts([
        { 
          ...
          rating: 4.5,
          description: 'Sample product when API is unavailable.' 
        }
      ]);
    } finally {
      setLoading(false);
...
// Fetch user cart from Fake Store API
useEffect(() => {
  const fetchUserCart = async () => {
    try {
      setCartLoading(true);
      
      // Fetch user's cart (using user ID 1 as example)
...
// Example: Fetch single product (as shown in your example)
...
// State-dependent functions will be declared after state initialization
  ...
  const [orders, setOrders] = useState([
    ...
    { id: ' ORD-005', date: '2025-07-01', total: 89.99, status: 'Out for Delivery', tracking: 'In Transit', items: 3, paymentMethod: 'visa' },
    { id: ' ORD-004', date: '2025-05-28', total: 45.50, status: 'Delivered', tracking: 'Completed', items: 2, paymentMethod: 'mpesa' },
    { id: ' ORD-005', date: '2025-05-11', total: 89.99, status: 'Out for Delivery', tracking: 'In Transit', items: 3, paymentMethod: 'visa' },
    {id: ' ORD-004', date: '2025-05-05', total: 45.50, status: 'Delivered', tracking: 'Completed', items: 2, paymentMethod: 'mpesa' },
        ...
        <div style={{ padding: '1.5rem' }}>
          {activeSection === 'dashboard' && (
            <>
              ...
              // Add to your dashboard section (after Welcome Banner)
